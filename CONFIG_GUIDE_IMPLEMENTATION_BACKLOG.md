...
- `F-06` (`partial`): Zone workflow hardening in progress (cross-zone validation added: interface overlap protection + `from_zone` existence/canonicalization with `LOCAL`; UI now pre-validates policy textarea format/duplicates before apply; remaining onboarding/UX depth pending).
