...
export interface VtiInterfaceConfig {
  name: string;
  description: string;
  addresses: string[];
  mtu: string;
...
export interface VtiConfig {
  interfaces: VtiInterfaceConfig[];
}

class VtiService {
  private readonly api = new ConfigTreeApi("vti-interface", "vti");

  async getRawConfig(refresh = false): Promise<Record<string, unknown>> {
    return this.api.getConfig<Record<string, unknown>>(refresh);
  }

  async getConfig(refresh = false): Promise<VtiConfig> {
    const root = asObject(await this.getRawConfig(refresh));
    const interfaces: VtiInterfaceConfig[] = [];
