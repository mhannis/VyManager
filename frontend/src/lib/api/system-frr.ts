...
export interface SystemFrrConfig {
  bmp: boolean;
  descriptors: string;
  irdp: boolean;
  profile: string;
...
class SystemFrrService {
  private readonly api = new ConfigTreeApi("system-frr", "frr");

  async getRawConfig(refresh = false): Promise<Record<string, unknown>> {
    return this.api.getConfig<Record<string, unknown>>(refresh);
  }

  async getConfig(refresh = false): Promise<SystemFrrConfig> {
    const root = asObject(await this.getRawConfig(refresh));
    return {
      bmp: Object.prototype.hasOwnProperty.call(root, "bmp"),
      descriptors: asString(root.descriptors),
      irdp: Object.prototype.hasOwnProperty.call(root, "irdp"),
      profile: asString(root.profile),
      snmpDaemons: readTagValues(root.snmp),
    };
  }
...
export const systemFrrService = new SystemFrrService();
