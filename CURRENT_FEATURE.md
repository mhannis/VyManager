feature_id: firewall-global-options-batch-validation-hardening-2026-02-16
title: Firewall global-options batch validation hardening (`F-04` depth pass)
status: done
branch: feature/containers-automation-v1
completed_in_cycle:
  - Hardened `POST /vyos/firewall/global-options/batch` with strict per-operation validation and normalization.
  - Added parameter-arity enforcement so operations now fail with explicit `400` for missing required values or unexpected values on no-arg operations.
  - Added enum validation for enable/disable, source validation, state-policy action, and state-policy log-level batch operations.
  - Added timeout validation for `set_timeout_*` operations (integer + range), with deterministic `400` errors for invalid values.
  - Added regression tests in `backend/tests/test_firewall_global_options_batch_validation.py` for invalid batch payloads and valid normalized behavior.
validation:
  - cd backend && PYTHONPATH=. ./.venv/bin/pytest -q tests/test_firewall_global_options_batch_validation.py tests/test_firewall_global_options_validation.py tests/test_firewall_zones_validation.py tests/test_firewall_zones_local_zone.py tests/test_firewall_flowtables_validation.py tests/test_firewall_batch_semantics.py tests/test_firewall_groups_validation.py tests/test_firewall_nat_save_apply_reload_loops.py tests/test_firewall_nat_config_snapshots.py
  - cd frontend && npx tsc --noEmit --pretty false
known_limitations:
  - Frontend smoke/build gates were not rerun this backend-only slice; latest prior cycle state remains green (build + runtime smoke pass, lint warnings only).
  - Browser smoke (Playwright) remains blocked on host dependency (`libnspr4.so`) and is outside this slice’s pass gate.
next_queue:
  - Continue firewall depth work (`F-01`, `F-02`) with additional option-level UX and command coverage.
  - Continue zone workflow onboarding and UX depth (`F-06`) on top of the new cross-zone validation baseline.
  - Continue interfaces-depth sweep (`IF-15`) with validation and fixture-backed verification.
  - Continue service parity depth (`SVC-03`, `SVC-04`, `SVC-05`) with guide-complete forms and test expansion.
