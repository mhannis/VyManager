...
Thin API wrappers for VyOS PIM protocol configuration.
"""
...
class PimCapabilitiesResponse(BaseModel):
    protocol: str
    version: str
...
class PimConfigResponse(BaseModel):
    pim: Dict[str, Any] = Field(default_factory=dict)
...
def _validate_operations(operations: List[str]) -> None:
    ...
    for command in operations:
        ...
        if not any(cleaned == root or cleaned.startswith(f"{root} ") for root in allowed_roots):
            raise HTTPException(
                status_code=400,
                detail="PIM batch only allows commands under 'protocols pim'",
            )


@router.get("/capabilities", response_model=PimCapabilitiesResponse)
async def get_pim_capabilities(request: Request):
    await require_read_permission(request, FeatureGroup.PIM)

    try:
        service = get_session_vyos_service(request)
        capabilities = PimCapabilitiesResponse(
            protocol="pim",
            version=service.get_version(),
            config_path=["protocols", "pim"],
            features={
                "supports_batch": True,
...
@router.get("/config", response_model=PimConfigResponse)
async def get_pim_config(request: Request, refresh: bool = False):
    await require_read_permission(request, FeatureGroup.PIM)
    ...
    try:
        ...
        return PimConfigResponse(pim=pim_config)
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
async def pim_batch_configure(request: Request, body: PimBatchRequest):
    await require_write_permission(request, FeatureGroup.PIM)
