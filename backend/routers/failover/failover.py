"""
Failover Protocol Router

Thin API wrappers for VyOS Failover protocol configuration.
"""
...
class FailoverCapabilitiesResponse(BaseModel):
    protocol: str
    version: str
...
def _validate_operations(operations: List[str]) -> None:
    ...
    for command in operations:
        ...
        if not cleaned.startswith(allowed_prefixes):
            raise HTTPException(
                status_code=400,
                detail="Failover batch only allows commands under 'protocols failover'",
            )


@router.get("/capabilities", response_model=FailoverCapabilitiesResponse)
async def get_failover_capabilities(request: Request):
    await require_read_permission(request, FeatureGroup.FAILOVER)

    try:
        service = get_session_vyos_service(request)
        capabilities = FailoverCapabilitiesResponse(
            protocol="failover",
            version=service.get_version(),
            config_path=["protocols", "failover"],
            features={
                "supports_batch": True,
...
@router.get("/config", response_model=FailoverConfigResponse)
async def get_failover_config(request: Request, refresh: bool = False):
    await require_read_permission(request, FeatureGroup.FAILOVER)
    ...
    try:
        ...
        return FailoverConfigResponse(failover=failover_config)
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
async def failover_batch_configure(request: Request, body: FailoverBatchRequest):
    await require_write_permission(request, FeatureGroup.FAILOVER)
