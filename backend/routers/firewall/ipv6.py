...
ICMPV6_SET_OPS = {"set_rule_icmpv6_type_name"}
LEGACY_ALIASES = {
...
def _canonical_operation_name(operation_name: str) -> str:
    return LEGACY_ALIASES.get(operation_name, operation_name)


def _validate_batch_semantics_or_400(operations: List["FirewallBatchOperation"]) -> None:
    canonical_ops = [(_canonical_operation_name(operation.op), operation.value) for operation in operations]
    op_names = {name for name, _ in canonical_ops}
    action_value: Optional[str] = None
    protocol_value: Optional[str] = None
    protocol_inverted = False

    for op_name, op_value in canonical_ops:
        if op_name == "set_rule_action" and op_value is not None and op_value.strip():
            action_value = op_value.strip().lower()
        if op_name == "set_rule_protocol" and op_value is not None and op_value.strip():
            protocol_value, protocol_inverted = _normalize_protocol_value(op_value)
    ...
    if "set_rule_tcp_flags" in op_names and "set_rule_icmpv6_type_name" in op_names:
        raise HTTPException(
            status_code=400,
            detail="TCP flags and ICMPv6 type matching cannot be set in the same batch",
        )

    if protocol_value is None:
        return

    if protocol_inverted and bool(op_names.intersection(PORT_SET_OPS | TCP_FLAGS_SET_OPS | ICMPV6_SET_OPS)):
        raise HTTPException(
            status_code=400,
            detail="Inverted protocol matching cannot be combined with port/TCP/ICMPv6 set operations in the same batch",
        )
    ...
    if op_names.intersection(ICMPV6_SET_OPS) and protocol_value != "ipv6-icmp":
        raise HTTPException(status_code=400, detail="ICMPv6 type matching requires protocol ipv6-icmp")
...
async def firewall_ipv6_batch_configure(http_request: Request, request: FirewallBatchRequest):
    ...
    try:
        ...
        for operation in request.operations:
            method_name = _canonical_operation_name(operation.op)
