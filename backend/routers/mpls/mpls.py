...
Thin API wrappers for VyOS MPLS protocol configuration.
"""
...
class MPLSCapabilitiesResponse(BaseModel):
    protocol: str
    version: str
...
class MPLSConfigResponse(BaseModel):
    mpls: Dict[str, Any] = Field(default_factory=dict)
...
def _validate_operations(operations: List[str]) -> None:
    ...
    for command in operations:
        ...
        if not cleaned.startswith(allowed_prefixes):
            raise HTTPException(
                status_code=400,
                detail="MPLS batch only allows commands under 'protocols mpls'",
            )


@router.get("/capabilities", response_model=MPLSCapabilitiesResponse)
async def get_mpls_capabilities(request: Request):
    await require_read_permission(request, FeatureGroup.MPLS)

    try:
        service = get_session_vyos_service(request)
        capabilities = MPLSCapabilitiesResponse(
            protocol="mpls",
            version=service.get_version(),
            config_path=["protocols", "mpls"],
            features={
                "supports_batch": True,
...
@router.get("/config", response_model=MPLSConfigResponse)
async def get_mpls_config(request: Request, refresh: bool = False):
    await require_read_permission(request, FeatureGroup.MPLS)

    try:
        service = get_session_vyos_service(request)
        full_config = await run_in_threadpool(service.get_full_config, refresh=refresh)
        mpls_config = full_config.get("protocols", {}).get("mpls", {})
        return MPLSConfigResponse(mpls=mpls_config)
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
async def mpls_batch_configure(request: Request, body: MPLSBatchRequest):
    await require_write_permission(request, FeatureGroup.MPLS)
