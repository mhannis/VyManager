"""Shared helper for VPN subtree wrapper routers."""
...
def build_vpn_router(
    *,
    ...
    class VpnBatchRequest(BaseModel):
    ...
    def validate_operations(operations: List[str]) -> None:
        ...
        for command in operations:
            ...
            if not any(cleaned == root or cleaned.startswith(f"{root} ") for root in allowed_roots):
                raise HTTPException(
                    status_code=400,
                    detail=f"{display_name} batch only allows commands under 'vpn {vpn_name}'",
                )
    ...
    async def get_config(request: Request, refresh: bool = False):
        ...
        try:
            ...
            vpn_config = full_config.get("vpn", {}).get(vpn_name, {})
            return VpnConfigResponse(vpn=vpn_config)
        except HTTPException:
            raise
        except Exception as exc:
            raise HTTPException(status_code=500, detail=str(exc))
    ...
    async def batch_configure(request: Request, body: VpnBatchRequest):
